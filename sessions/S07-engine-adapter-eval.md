...
# S07 · Adapter evaluation and the null result

**System 01 · Legal Intelligence Engine**

**Goal.** Build the output parser, test on five examples, confirm non-zero before running all forty. Score on the same golden set. **Compute the interval before writing any comparative sentence.** Publish two models with cards.

**Commit.** `feat(legal-engine): evaluate all adapters on the golden set with intervals, publish two models`

**Evidence.** `Ablation table with 95% intervals · 2 model pages live`
